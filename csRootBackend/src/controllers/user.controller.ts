...
import uploadProfilePicture from "../service/uploadProfilePicture";
...
const verifyEmail = async (req: Request, res: Response) => {
  ...
  const registeredUserData = await redis.get(`registerationData:${email}`);
...
const login = async (req: Request, res: Response) => {
  ...
  if (user.twoStepVerification) {
    ...
    return res.status(200).json(
      new apiResponse(200, "Please confirm your account", {
...
const verifyTwoStepVerification = async (req: Request, res: Response) => {
  ...
  const storedCode = await redis.get(`twoStepVerification:code:${user.email}`);
  ...
  await redis.del(`twoStepVerification:code:${user.email}`);
...
const changeProfilePicture = async (
  ...
  if (
    ...
    oldProfilePicturePublicId !== result.public_id
  ) {
    try {
  ...
  return res
    ...
    .json(
      new apiResponse(200, "Profile picture updated successfully", {
        ...
        profilePicturePublicId: req.user.profilePicturePublicId,
      }),
    );
};
